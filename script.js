...
{professions = professions + "es"} else if (professions == 'man') { professions = 'men'}  else if (professions == 'woman') { professions = 'women'} else {
...
{professions2 = professions2 + "es"} else if (professions2 == 'man') { professions2 = 'men'}  else if (professions2 == 'woman') { professions2 = 'women'} else {
...
professions3 = professions3 + "s";    
  ...
  var negativeadjectivelist = ['shitty', 'weak', 'minor', 'delicate', 'ignorant', 'oblivious', 'cowardly', 'scared', 'cross eyed', 'electric', 'amped up', 'crazed', 'insane', 'ridiculous', 'shoddy', 'subpar', 'substandard', 'cheap', 'rough', 'dirty', 'unclean', 'lame', 'broken', 'hallucinating', 'destructive', 'amphibious', 'insane', 'unclean','pcychotic','horny','blind','angry','furious','frustrated','twisted','supreme','dominant','submissive','kinky','naked','sexual','naughty','half dressed','braindead','insignificant','eye rolling','sarcastic','unreal','comical','garbage like','unbelievable','blind','reckless','clumsy','circus like','inbred','masochistic','sadistic','corny','savage'];
...
{niceobjects = niceobjects + "es"} else {
...
{niceobjects2 = niceobjects2 + "es"} else {
...
{niceobjects3 = niceobjects3 + "es"} else {
niceobjects3 = niceobjects3 + "s";    
};


  var list = niceobjectlist.concat(['auto rifle','shotgun','sniper rifle','rocket launcher','grenade launcher','bow','scout rifle','revolver','pistol','fork', 'nailgun', 'chainsaw', 'sword', 'dagger', 'spear', 'bone', 'club', 'grinder', 'engine block', 'exhaust pipe', 'muffler','piece of trash', 'fragment', 'trashbag', 'crowbar', 'air pump', 'water pump', 'tobacco pipe', 'toilet', 'motor oil bottle', 'jackhammer', 'hatchet', 'fireworks display','hammer','mixer','knife','blade','railroad spike','boulder','piece of flint','pipe cutter','stake','empty can','fire extinguisher','power outlet','water dish','powercord','candle','torch','dildo','bong','hooka','keg of alcohol']);
...
{enjoyancewords = enjoyancewords + "es"} else if (enjoyancewords[enjoyancewords.length - 2] + enjoyancewords[enjoyancewords.length - 1] == 'cy') {
...
{locations2 = locations2 + "es"}   else if (locations2[locations2.length - 1] == 's'){} else {
...
locations3 = locations3 + "s";    
  ...
  if (questionorstatement == 'random'){
    var list = ['animal','being','famousbeing','object'];
  ...
  var list = ['Vayne','Shyvanna','Taliyah','Ahri','Kayle','Morganna','Malphite','Master Yi','Imperius','Diablo','Azmodan','Tyrael','Cayde-6', 'Ikora', 'Zavala', 'Britney Spears', 'Elvis Presley', 'Bruce Campbell', 'Banshee-44', 'Xur', 'the Drifter', 'Lotus', 'Thrall', 'Jaina', 'Jin Raynor', 'Alarak', 'Vorazhun', 'Zeratul', 'Stukov', 'Zagara', 'the Overmind', 'a Banshee','a Mag','an Ember','a Nova','a Titania','a Trinity', 'Samus', 'Link', 'Zelda','Ada-1','Amanda Holliday','Ana Bray','Lord Shaxx','Asher Mir','Benedict 99-40','Brother Vance','Darbi 55-30','Devrim Kay','Dominus Ghaul','Eris Morn','Eva Levante','Executor Hideo','Failsafe','Kadi 55-30', 'Lakshmi-2', 'Mara Sov','Osiris','Petra Venj','Rasputin','Shuro Chi','Sedia','Sloane','Suraya Hawthorne','Tess Everis','Spider','Uldren Sov','Val Cauor','Xol','Ordis','Darvo','Maroo','Teshin','Clem','Cephalon Cy','Cephalon Simaris','Spock','Picard','Kirk','Scotty','Leonard McCoy','Geordi La Forge','Sulu','Tuvok','Chekov','7 of 9','Janeway','a Grineer','a Infested','a Corpus','the Void'];
    ...
    if (referencetype == 'at location') {
      ...
      if (phrasetype == 'question') {
        ...
        switch (diceroll) {
          ...
          case 3:
          var resultphrase = ('Do ' + randompercentage1 + ' of all ' + locationz + ' have ' + adjective1+ ' ' + objects + '?')
